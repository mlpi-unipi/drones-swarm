...
bool px4_msgs__msg__wheel_encoders__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  ...
  {  // wheel_speed
    ...
    }
    {
      ...
      Py_ssize_t size = 2;
      float * dest = ros_message->wheel_speed;
  ...
  {  // wheel_angle
    ...
    }
    {
      ...
      assert(PyArray_TYPE(seq_field) == NPY_FLOAT32);
      Py_ssize_t size = 2;
      float * dest = ros_message->wheel_angle;
      for (Py_ssize_t i = 0; i < size; ++i) {
        float tmp = *(npy_float32 *)PyArray_GETPTR1(seq_field, i);
        memcpy(&dest[i], &tmp, sizeof(float));
      }
...
PyObject * px4_msgs__msg__wheel_encoders__convert_to_py(void * raw_ros_message)
{
  ...
  {  // wheel_speed
    ...
    float * src = &(ros_message->wheel_speed[0]);
    memcpy(dst, src, 2 * sizeof(float));
    Py_DECREF(field);
  }
  {  // wheel_angle
    PyObject * field = NULL;
    field = PyObject_GetAttrString(_pymessage, "wheel_angle");
    if (!field) {
      return NULL;
    }
    assert(field->ob_type != NULL);
    ...
    assert(sizeof(npy_float32) == sizeof(float));
    npy_float32 * dst = (npy_float32 *)PyArray_GETPTR1(seq_field, 0);
    float * src = &(ros_message->wheel_angle[0]);
    memcpy(dst, src, 2 * sizeof(float));
    Py_DECREF(field);
  }
