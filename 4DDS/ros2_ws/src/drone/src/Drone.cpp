...
void fdds::Drone::bootstrap()
{
    ...
    {
        sendGoto(Eigen::Vector2d{}, odometryZDownMetres);
        return;
    }
...
{    
    ...
    // Controlla il tipo di veicolo
    if (vehicleType == "iris") {

        // Controlla l'altitudine per il decollo
        if (-odometryZDownMetres < -options.desiredRelativeAltitude - 0.5) {

            requestTakeoff(options.desiredRelativeAltitude);
            sendGeoPing();
            takeoff = false;
            return;
        }
        ...
        // Crea la matrice di rotazione convertendo yawRadians in float
        ...
        // Controlla la restrizione della velocità massima
        ...
        sendVelocity(currentSpeed, options.desiredRelativeAltitude);
    }
        ...
        sendVelocity(currentSpeed, options.desiredRelativeDepth);

    }
    else {
        std::cout << "Tipo di veicolo sconosciuto: " << vehicleType << "\n";
    }
...
void fdds::Drone::publishTargetPosition()
{
    ...
    std::cout << std::setw(20) << "Vehicle ID:" << " " << static_cast<int>(msg.vehicle_id) << "\n";
    std::cout << std::setw(20) << "Latitude (m):" << " " << msg.latitude_m << "\n";
    std::cout << std::setw(20) << "Longitude (m):" << " " << msg.longitude_m << "\n";
    std::cout << std::setw(20) << "Speed North (m/s):" << " " << msg.speed_north_m_s << "\n";
    std::cout << std::setw(20) << "Speed East (m/s):" << " " << msg.speed_east_m_s << "\n\n";
}
...
void fdds::Drone::heartbeat()
{
    ...
    /*
    std::cout << "OFFBOARD CONTROL:\n";
    std::cout << std::setw(20) << "Timestamp (µs):" << " " << msg.timestamp << "\n";
    std::cout << std::setw(20) << "Position Control:" << " " << (msg.position ? "Enabled" : "Disabled") << "\n";
    std::cout << std::setw(20) << "Velocity Control:" << " " << (msg.velocity ? "Enabled" : "Disabled") << "\n";
    std::cout << std::setw(20) << "Acceleration Control:" << " " << (msg.acceleration ? "Enabled" : "Disabled") << "\n";
    std::cout << std::setw(20) << "Attitude Control:" << " " << (msg.attitude ? "Enabled" : "Disabled") << "\n";
    std::cout << std::setw(20) << "Body Rate Control:" << " " << (msg.body_rate ? "Enabled" : "Disabled") << "\n";
    std::cout << std::setw(20) << "Thrust & Torque Control:" << " " << (msg.thrust_and_torque ? "Enabled" : "Disabled") << "\n";
    std::cout << std::setw(20) << "Direct Actuator Control:" << " " << (msg.direct_actuator ? "Enabled" : "Disabled") << "\n\n";
    */
}
...
*/
void fdds::Drone::sendGoto(const Eigen::Vector2d& location, float value)
{
    px4_msgs::msg::TrajectorySetpoint msg;
    msg.position = std::array<float, 3>{float(location(0)), float(location(1)), value};
    msg.timestamp = get_clock()->now().nanoseconds() / 1000;
    vehicleTrajectoryPublisher->publish(msg);

    std::cout << "GOTO MSG:\n";
    std::cout << "  Timestamp: " << msg.timestamp << " µs\n";
    std::cout << "  Position (NED): [" << msg.position[0] << ", " << msg.position[1] << ", " << msg.position[2] << "] m\n";
    std::cout << "  Velocity (NED): [" << msg.velocity[0] << ", " << msg.velocity[1] << ", " << msg.velocity[2] << "] m/s\n";
    std::cout << "  Acceleration (NED): [" << msg.acceleration[0] << ", " << msg.acceleration[1] << ", " << msg.acceleration[2] << "] m/s^2\n";
    std::cout << "  Jerk (NED): [" << msg.jerk[0] << ", " << msg.jerk[1] << ", " << msg.jerk[2] << "] m/s^3\n";
    std::cout << "  Yaw: " << msg.yaw << " rad\n";
    std::cout << "  Yawspeed: " << msg.yawspeed << " rad/s\n\n";
}
...
*/
void fdds::Drone::sendVelocity(const Eigen::Vector2d& velocity, float value)
{
    px4_msgs::msg::TrajectorySetpoint msg;
    msg.velocity = std::array<float, 3>{float(velocity(0)), float(velocity(1)), NAN};

    msg.position = std::array<float, 3>{10,10,0};
    msg.velocity = std::array<float, 3>{10,10,0};
    msg.acceleration = std::array<float, 3>{10,10,0};
    msg.jerk = std::array<float, 3>{10,10,0};
    msg.yaw = 10;
    msg.yawspeed = 10;
...
*/
void fdds::Drone::requestTakeoff(float value)
{
    sendGoto(odometryPositionMetres, value);
}
